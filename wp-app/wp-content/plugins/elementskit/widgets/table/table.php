...
class Elementskit_Widget_table extends Widget_Base {
    ...
    protected function render_raw( ) {
        ...
        $table_settings = [
            ...
            'prev_arrow'         => ($is_new_prev || $migrated_prev) ? trim($settings['ekit_table_navigation_prev_arrows']['value']) : trim($settings['ekit_table_navigation_prev_arrow']),
            'next_arrow'         => ($is_new_next || $migrated_next) ? trim($settings['ekit_table_navigation_next_arrows']['value']) : trim($settings['ekit_table_navigation_next_arrow'])
        ];
                   ...
                   foreach((array)$ekit_table_build_header as $head){
                       ...
                       if ( $head['cell_icon_type'] != 'none') {
                           ...
                           if ( $head['cell_icon_type'] == 'icon' && $head['cell_icons'] != '' ) {
                               $head_icon .= ( $is_new || $migrated ) ? '<i class="'. esc_attr( $head['cell_icons']['value'] ) .'"></i>'  :  '<i class="'. esc_attr( $head['cell_icon']) .'"></i>';
                    ...
                    foreach((array)$table_body_content as $bodycon){
                       ...
                       if ( $bodycon['body_cell_icon_type'] != 'none') {
                           ...
                           if ( $bodycon['body_cell_icon_type'] == 'icon' && $bodycon['body_cell_icons'] != '' ) {
                               $body_icon .= ( $is_new || $migrated ) ? '<i class="' . esc_attr( $bodycon['body_cell_icons']['value'] ) . '"></i>' : '<i class="' . esc_attr( $bodycon['body_cell_icon']) . '"></i>'; 
                            ...
                            <td data-order="<?php echo esc_attr($bodycon['cell_text']); ?>" class="elementor-repeater-item-<?php echo esc_attr($bodycon['_id']); ?> ekit_table_data_<?php echo esc_attr($bodycon['cell_icon_position']); ?>">
                                ...
                                <?php endif; ?>
                                    ...
                                    <div class="ekit_table_body_container ekit_table_data_<?php echo esc_attr($bodycon['cell_icon_position']); ?> ekit_body_align_<?php echo esc_attr($settings['body_align']); ?>">
                                        <?php echo \ElementsKit\Utils::render($body_cell_data); ?>
                                    </div>
