...
class Elementskit_Widget_Woo_Product_Carousel extends Widget_Base {
	...
	protected function render_both_navigation() {
		$settings = $this->get_settings();
			...
			$prevIcon = ( $is_new || $migrated ) ? $settings['ekit_arrows_prev_icons']['value'] : $settings['ekit_arrows_prev_icon'];

			// new icon
			$migrated = isset( $settings['__fa4_migrated']['ekit_arrows_next_icons'] );
			// Check if its a new widget without previously selected icon using the old Icon control
			$is_new = empty( $settings['ekit_arrows_next_icon'] );
			$nextIcon = ( $is_new || $migrated ) ? $settings['ekit_arrows_next_icons']['value'] : $settings['ekit_arrows_next_icon'];
		?>
		...
		<script>
			jQuery(document).ready(function($) {
			    ...
			    var swiper = new Swiper("#<?php echo esc_attr($id); ?> .ekit-swiper-container", {
			        ...
			        "autoplay"      : <?php echo \ElementsKit\Utils::render(($settings['ekit_autoplay'] == 'yes') ? '{ "delay": ' . $settings['ekit_autoplay_speed'] . ' }' : 'false'); ?>,
			        "loop"          : <?php echo \ElementsKit\Utils::render($settings['ekit_loop'] == 'yes' ? 'true' : 'false'); ?>,
			        "speed"         : <?php echo \ElementsKit\Utils::render($settings['ekit_speed']['size']*10); ?>,
